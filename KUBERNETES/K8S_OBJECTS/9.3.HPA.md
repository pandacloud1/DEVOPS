# 📈 HORZONTAL POD AUTOSCALER (HPA)
## 📌 Note:
1. Ensure that Metrics Server is installed in your cluster before using HPA.

## METRICS SERVER
Metrics server is a pod that collects resource usage data from nodes & pods.<br>
Mostly used to check CPU/RAM, monitoring, autoscaling, etc.

Check if metrics server is present in your cluster
```bash
